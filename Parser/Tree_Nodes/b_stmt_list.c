...
b_stmt_list *create_b_stmt_list(GArray *token_stream, unsigned long index, unsigned long *next) {
    if (g_array_index(token_stream, Token, index).type == t_start_bracket) {
        (*next)++;
    } else {
        fprintf(stderr,
                "Syntax Error: missing start bracket of body");
        exit(-1);
    }
    b_stmt_list *ret_val = create_b_stmt_list_helper(token_stream, index + 1, next);
    if (g_array_index(token_stream, Token, *next).type != t_end_bracket) {
        fprintf(stderr, "Syntax Error: missing end bracket of body");
        exit(-1);
    }
    (*next)++;
    return ret_val;
...
b_stmt_list *create_b_stmt_list_helper(GArray *token_stream, unsigned long index, unsigned long *next) {
    ...
    if (*next < token_stream->len && g_array_index(token_stream, Token, *next).type != t_end_bracket) {
        new_statement_list->next = create_b_stmt_list_helper(token_stream, *next, next);
    }
